...
export default function Calculateur() {
  ...
  return (
    <div className="px-4 py-6">
      <h2 className="font-display font-bold text-2xl text-gray-900 mb-6">{t('calc.title')}</h2>

      <Card className="mb-4">
        <label className="block text-xs font-medium text-gray-500 mb-1">{t('calc.rpDate')}</label>
        <input
          type="date"
          value={dateRP}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="mb-4 flex flex-col items-center py-6">
            ...
            <div className="text-center -mt-2">
              ...
              <p className="text-sm text-gray-500">{t('calc.daysOf', { required: DAYS_REQUIRED.toLocaleString() })}</p>
            </div>
            <div className="flex gap-3 mt-4">
              <Badge variant={pct >= 100 ? 'success' : 'brand'}>{pct}%</Badge>
              {remaining > 0 && <Badge variant="gray">{t('calc.remaining', { days: remaining })}</Badge>}
              {pct >= 100 && <Badge variant="success">{t('calc.eligible')}</Badge>}
            </div>
            {eligibleDate && remaining > 0 && (
              <p className="text-xs text-gray-500 mt-3">
                {t('calc.estimatedDate')} {format(eligibleDate, 'd MMMM yyyy', { locale: fr })}
              </p>
            )}
          </Card>

          <Card className="mb-4">
            <h3 className="font-display font-semibold text-base text-gray-900 mb-3">{t('calc.addAbsenceTitle')}</h3>
            <div className="flex flex-col gap-2">
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className="text-xs text-gray-500 mb-1 block">{t('calc.departure')}</label>
                ...
                <div className="flex-1">
                  <label className="text-xs text-gray-500 mb-1 block">{t('calc.return')}</label>
                  <input type="date" value={newAbsence.fin} onChange={e => setNewAbsence(a => ({...a, fin: e.target.value}))}
                    className="w-full p-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-brand-300" />
                </div>
              </div>
              <input placeholder={t('calc.destination')} value={newAbsence.destination}
              ...
              <Button onClick={addAbsence} disabled={!newAbsence.debut || !newAbsence.fin}>
                <Plus size={16} className="mr-1 inline" /> {t('calc.addBtn')}
              </Button>
            </div>
          </Card>
          ...
          {absences.length > 0 && (
            <Card>
              <h3 className="font-display font-semibold text-base text-gray-900 mb-3">
                {t('calc.logTitle')} <Badge variant="gray">{absenceDays} {t('calc.days')}</Badge>
              </h3>
              <div className="flex flex-col gap-2">
                {absences.map(a => (
